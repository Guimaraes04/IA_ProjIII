# Utilização do scpacy para extrair requisitos de um texto
...
# Função para extrair frases que contenham verbos
def extrair_frases_candidatas(doc):
    frases_candidatas = []
    for sent in doc.sents:
        if any(token.pos_ == "VERB" for token in sent):
            frases_candidatas.append(sent.text.strip())
    return frases_candidatas


# Extrair as frases retornadas pela função
frases_candidatas = extrair_frases_candidatas(doc)

for i, frase in enumerate(frases_candidatas, 1):
    print(f"{i}. {frase}")
